...
def capabilities(backend, profile):
    ...
    return {"text": True, "stream": True, "images": advanced, "tools": advanced,
            ...
            "max_inflight_limit": 1 if backend == "transformers" else 16,
            "unsupported_parameters": (["stop", "presence_penalty", "frequency_penalty"]
                                       if backend == "transformers" else [])}


def validate_capabilities(payload, backend, profile):
    caps = capabilities(backend, profile)
    ...
    if (images and not caps["images"]) or (tools and not caps["tools"]):
